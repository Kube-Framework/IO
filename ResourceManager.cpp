...
static Core::TrivialDispatcher<void(void), Core::Utils::DefaultStaticAllocator, Core::CacheLineQuarterSize> RegisterDispatcher {};
...
IO::Environment IO::ResourceManager::getEnvironment(const Core::HashedName environmentName) const noexcept
{
    ...
    const auto index = static_cast<std::uint32_t>(std::distance(_environmentNames.begin(), it));
